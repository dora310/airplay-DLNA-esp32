 ...
 *
 * Hooks into esp_log via esp_log_set_vprintf() and copies each line into a
 * non-blocking queue. A dedicated worker updates the browser ring buffer and
 * persistent diagnostics, keeping mutexes and file I/O out of audio tasks.
 * The Logs page drains lines from /api/logs/live. UART output is preserved.
 */
