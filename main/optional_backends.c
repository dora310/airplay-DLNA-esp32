...
esp_err_t optional_backends_init(void) {
  ...
  if (result != ESP_OK) ESP_LOGW(TAG, "Snapcast adapter unavailable: %s", esp_err_to_name(result));
#endif
#ifdef CONFIG_SQUEEZELITE_ADAPTER_ENABLE
  result = squeezelite_adapter_start();
  if (result != ESP_OK) ESP_LOGW(TAG, "Squeezelite adapter unavailable: %s", esp_err_to_name(result));
#endif
#ifdef CONFIG_BLE_PROVISIONING_ENABLE
  result = ble_provisioning_adapter_start();
  if (result != ESP_OK) ESP_LOGW(TAG, "BLE provisioning adapter unavailable: %s", esp_err_to_name(result));
#endif
